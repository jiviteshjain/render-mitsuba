from render_for_video_2 import Renderer
...
def run(args, frame_id, angle, box=None):
    ...
    renderer = Renderer(args.mitsuba, args.radius, args.radius, 1028, out_file_path, XML_HEAD, XML_TAIL, XML_SPHERE, scale_radius=True)
    ...
    for i, file in enumerate(args.default_color_files):
        if i == 0:
            renderer.add_pcd(os.path.join(args.in_dir, file), 'default', Rs[0].T @ Rs[0].T)
        elif i == 1:
            renderer.add_pcd(os.path.join(args.in_dir, file), 'default', np.eye(3))
        else:
            renderer.add_pcd(os.path.join(args.in_dir, file), 'default', Rs[0] @ Rs[0])
...
if __name__ == '__main__':
    ...
    box = None
    angle = 20 * np.pi / 180
    for frame_id, angle in enumerate(np.linspace(start=-angle, stop=angle, num=150)):
